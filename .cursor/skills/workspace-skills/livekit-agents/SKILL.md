...
4. **Plan tests for the right layer** - Voice worker and backend agent logic should have tests where practical (see testing section). Dashboard/UI wiring is usually verified manually unless the user requests frontend tests.
...
## Testing agent behavior (worker / server, not the dashboard by default)

Voice agent **runtime behavior** lives primarily in the worker (`apps/voice-agent`) and in backend endpoints that issue tokens or dispatch rooms. Those layers benefit from automated tests. **Do not add or require a frontend unit-test runner** (e.g. Vitest in `apps/dashboard`) for LiveKit UI unless the user explicitly asks.

### When to add tests

When building or modifying **LiveKit agent worker code** or **backend** pieces that affect calls:

1. **Use the existing `tests/` (or package test layout)** in `apps/voice-agent` / `apps/backend` as appropriate
2. **Add or extend tests** for logic you change (helpers, loaders, token/dispatch behavior) before considering the change complete
3. **Run** `pnpm --filter @bloomin-ai/voice-agent test` or `pnpm --filter @bloomin-ai/backend test` after substantive edits
...
### Why worker/server tests matter

Agents that "seem to work" in manual testing frequently fail in production: prompt changes, tool wiring, and model updates all shift behavior. Tests on **pure logic and integration boundaries** catch many regressions; full room simulation may still be manual.

### Skipping tests

If a user explicitly requests no tests, proceed without them but note that worker/backend coverage is still valuable for production. **Frontend-only** LiveKit wiring (e.g. `LiveKitRoom` in Next.js) is normally validated manually unless they ask for UI tests.
...
Prompt and tool changes are behavior changes. Prefer tests on the **worker** for helpers, config loading, and deterministic branches. **Do not** treat dashboard LiveKit components as requiring a new test harness by default.
...
5. **Test worker/backend behavior** where it pays off; validate dashboard flows manually unless the user wants UI tests
